...
export default function ChatInfoPage({ params }: PageProps) {
  ...
  const participants = participantHashes.map((hash) => {
    ...
    return profile
      ...
      : {
          hash,
          nickname:
            messages.find((m) => m.userhash === hash)?.usernick || "Unbekannt",
          fullname: "",
        };
  });
  ...
  const displayName = isPublic
    ? chatRoom.chatname
    : otherUser?.nickname || chatRoom.chatname;
              ...
              {chatRoom.joined ? (
                <>
                  ...
                  <button
                    ...
                    className="flex-1 bg-red-950 hover:bg-red-900 text-red-400 py-3 rounded-xl flex flex-col items-center gap-2 transition-colors shadow-sm border border-red-900/50"
                  >
                    <LogOut size={20} />
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-500 hover:to-orange-400 text-white py-3.5 rounded-xl flex items-center justify-center gap-2 font-semibold shadow-lg shadow-orange-900/20 transition-all"
                >
                  <LogIn size={20} />
                  Beitreten
                </button>
        ...
        <div className="mt-10 px-6 max-w-md mx-auto w-full">
          ...
          <div className="space-y-3">
            {participants.map((p) => (
              <div
                key={p.hash}
                className="flex items-center gap-4 p-3 bg-slate-900/50 rounded-xl border border-slate-800/50"
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center font-bold bg-gradient-to-br ${getAvatarColor(p.nickname)} text-sm shadow-sm`}
                >
